...
export function configureFilesystemRouter(appState: AppState): Router {
  ...
  async function getCargoFileProperties(
    ...
    try {
      ...
      const cargoFile = cargoFiles.find(
        (cargo) => path.normalize(cargo.targetPath) === normalizedPath,
      );
  ...
  router.get("/:serverId/contents/*", authMiddleware, async (req, res) => {
    try {
      ...
      if (isCargoFile) {
        ...
        res.setHeader(
          "X-Cargo-Readonly",
          properties?.readonly ? "true" : "false",
        );
      }
  ...
  router.delete("/:serverId/delete/*", authMiddleware, async (req, res) => {
    try {
      ...
      if (stats.isDirectory()) {
        ...
        const protectedFiles = cargoFiles.filter(
          (cargo) =>
            (cargo.targetPath.startsWith(dirPrefix) ||
              cargo.targetPath === targetPath) &&
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        const fullTargetDir = getValidatedPath(
          req.server.internalId,
          targetDir,
        );
        ...
        const results = await Promise.all(
          files.map(async (file) => {
            try {
              ...
              const conflictingCargoFile = cargoFiles.find(
                (cargo) =>
    ...
    async (req, res) => {
      try {
        ...
        try {
          ...
          const cargoFiles = await getCargoFiles(req.server.internalId);
          const readonlyCargoFiles = cargoFiles.filter(
            (cargo) => cargo.properties?.readonly,
          );
            ...
            for (const cargoFile of readonlyCargoFiles) {
              for (const extractedFile of extractedFiles) {
                if (cargoFile.targetPath === extractedFile) {
                  throw new Error(
                    `Extraction would overwrite readonly cargo file: ${cargoFile.targetPath}`,
                  );
                }
              }
            }
        ...
        res
          .status(500)
          .json({ error: "Failed to extract archive", message: error.message });
      }
    },
  );
  ...
  router.get("/:serverId/search", authMiddleware, async (req, res) => {
    try {
      ...
      const fullPath = getValidatedPath(
        req.server.internalId,
      ...
      async function searchFiles(dirPath: string) {
        ...
        for (const item of items) {
          ...
          const cargoFile = cargoFiles.find(
            (cargo) => cargo.targetPath === relativePath,
          );
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        const destinationPath = getValidatedPath(
          req.server.internalId,
          destination,
        );
        if (!destinationPath) {
