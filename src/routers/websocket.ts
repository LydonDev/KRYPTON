...
export class WebSocketManager {
  ...
  private async handleSendCommand(session: ConsoleSession, command: string) {
    ...
    try {
      ...
      console.log(
        "[Command Handler] Starting command execution:",
        sanitizedCommand,
      );

      const { spawn } = require("child_process");

      const dockerAttach = spawn(
        "docker",
        ["attach", "--sig-proxy=false", session.container.id],
        {
  ...
  private async handlePowerAction(session: ConsoleSession, action: string) {
    ...
    try {
      this.broadcastToServer(
        session.internalId,
        `Performing a ${action} action on server...`,
        LogType.DAEMON,
      );

      const containerInfo = await session.container.inspect();
      const currentState = containerInfo.State.Status.replace(
        "exited",
        "stopped",
      );
      ...
      this.broadcastToServer(
        session.internalId,
        `Failed to ${action} server: ${errorMsg}`,
        LogType.ERROR,
      );
  ...
  private configureWebSocketRouter() {
    this.appState.wsServer.on(
      ...
      async (socket: WebSocket, request: any) => {
        ...
        const session = await this.setupContainerSession(
          socket,
          internalId,
          validation,
        );
  ...
  private async setupContainerSession(
    socket: WebSocket,
    internalId: string,
    validation: ValidateResponse,
  ) {
    try {
      ...
      if (!server?.docker_id) {
        console.error(
          `[WebSocket] No docker_id found for server ${internalId}`,
        );
